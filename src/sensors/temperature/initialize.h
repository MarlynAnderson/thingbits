...
void transmitString(char* transmission) {
  ...
  if ((transmissionLength + 1) > VW_MAX_PAYLOAD) {
      strncat_P(transmission, transmission, (VW_MAX_PAYLOAD - 1));
      transmissionLength = (VW_MAX_PAYLOAD - 1);
  }

  char checkSumByte = makeCheckSumByte(transmission, transmissionLength);
  transmission[transmissionLength] = checkSumByte;
  transmission[transmissionLength + 1] = 0;
  
  digitalWrite(pin_LED, HIGH);
  digitalWrite(pin_ANTENNA_POWER, HIGH);
  
  vw_send((uint8_t *)transmission, (transmissionLength + 1)); 
  vw_wait_tx();
